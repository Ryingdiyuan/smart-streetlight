<template>
  <section class="page-section">
    ...
    <div class="content-grid two-columns">
      <PanelCard v-if="device" title="基础信息" :subtitle="`设备 ID：${route.params.id}`">
      ...
      <PanelCard v-if="device" title="控制与阈值" subtitle="支持真实阈值保存与控制命令">
        ...
        <div class="button-row">
          <button class="primary-button" @click="saveThreshold">保存阈值</button>
    ...
    <div v-if="device" class="content-grid two-columns">
      ...
      <PanelCard title="运维建议" subtitle="结合当前阈值和设备状态给出提醒">
        <div class="detail-summary-grid">
          <div class="summary-box">
            <strong>巡检建议</strong>
...
const integrationSuggestion = computed(() => {
  if (!device.value) return "等待设备数据";
  if (!device.value.commandLogs.length) return "后续接真实接口时优先联调控制日志回写。";
  if (device.value.commandLogs.some((log) => log.result === "pending")) {
    return "存在 pending 指令，后端联调时建议补执行结果轮询。";
  }
  return "后续可优先接设备详情聚合接口与告警接口。";
});
