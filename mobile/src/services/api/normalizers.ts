...
export function mapAlarmType(type: string): AlarmType {
  ...
  if (normalized === "COMMAND_FAILED") return "COMMAND_FAILED";
  if (normalized === "LIGHT_ABNORMAL") return "LIGHT_ABNORMAL";
...
export function mapLightHistoryPoint(
  payload: LightDataApiPayload,
): LightHistoryPoint {
  return {
...
export function mapAlarmPayload(
  payload: AlarmApiPayload,
  deviceCode?: string,
): AlarmRecord {
  return {
    id: String(payload.id),
