...
async function getLatestLight(deviceId: number) {
  try {
    return await http.get<LightDataApiPayload>(
      `/devices/${deviceId}/latest-light`,
    );
  } catch (error) {
...
async function getLightHistory(deviceId: number) {
  try {
    return await http.get<LightDataApiPayload[]>(
      `/devices/${deviceId}/light-history`,
    );
  } catch (error) {
...
async function getDeviceCommands(deviceId: number) {
  try {
    return await http.get<ControlLogApiPayload[]>(
...
export async function getDeviceList(): Promise<Device[]> {
  ...
  return devices.map((device, index) => {
    ...
    if (
      latestLightResult.status !== "fulfilled" || !latestLightResult.value
    ) {
      return mappedDevice;
    }
...
export async function getDeviceDetail(
  ...
  try {
    ...
    return {
      ...device,
      lampStatus: latestLight
        ? mapLampStatus(latestLight.lamp_status)
        : device.lampStatus,
      ...
      commandLogs: commandLogs.map((item) =>
        mapCommandLogPayload(item, device.deviceCode)
      ),
      alarms: alarms
...
export async function updateDeviceThreshold(
  ...
  const payload = await http.put<ThresholdConfigApiPayload>(
...
export async function sendDeviceCommand(
  ...
  const payload = await http.post<ControlLogApiPayload>(
    `/devices/${id}/commands`,
    { command },
  );
...
export async function sendBatchDeviceCommand(
  ...
  const payload = await http.post<BatchControlLogApiPayload>(
    "/devices/commands/batch",
    { device_ids: deviceIds, command },
  );
