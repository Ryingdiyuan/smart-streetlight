...
export async function getRealtimeLightReadings(): Promise<
...
export async function getAllDevicesLightHistory(): Promise<
  Record<number, LightHistoryPoint[]>
> {
  const devices = await getDeviceList();
  const historyEntries = await Promise.all(
    devices.map(
      async (device) =>
        [device.id, await getLightHistory(device.id)] as const,
    ),
  );
