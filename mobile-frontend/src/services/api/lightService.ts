...
export async function getRealtimeLightReadings(): Promise<RealtimeLightReading[]> {
  ...
  const latestResults = await Promise.allSettled(devices.map((device) => getLatestLight(device.id)));
...
export async function getLightHistory(deviceId: number): Promise<LightHistoryPoint[]> {
  try {
    const history = await http.get<LightDataApiPayload[]>(`/devices/${deviceId}/light-history?limit=24`);
